...
BEGIN
    RAISERROR(N'You cannot deploy this update script to target XPS. The database for which this script was built, Scorepedia, does not exist on this server.', 16, 127) WITH NOWAIT
    RETURN
END
...
IF (@@servername != 'XPS')
BEGIN
    RAISERROR(N'The server name in the build script %s does not match the name of the target server %s. Verify whether your database project settings are correct and whether your build script is up to date.', 16, 127,N'XPS',@@servername) WITH NOWAIT
    RETURN
END
...
GO
PRINT N'Dropping [scorepedia].[GetPlayerById_MultiStatement]...';


GO
DROP FUNCTION [scorepedia].[GetPlayerById_MultiStatement];
